 ...
 *   public/index.html — LP1 placeholder text, Lilly SSO label, pharma AI prompt
...
patch("public/index.html", "LP1 org alignment placeholder", content =>
  replace(
    content,
    'placeholder="e.g. Business Unit Build Out, Corporate Safety..."',
    'placeholder="e.g. LP1 Build Out, Corporate Safety..."'
  )
);

patch("public/index.html", "Lilly SSO label in settings", content =>
  replace(
    content,
    "✓ AI features enabled",
    "✓ AI connected via Lilly SSO (lilly-code)"
  )
);

patch("public/index.html", "LP1 section placeholder", content =>
  replace(
    content,
    'placeholder="e.g. Work Projects"',
    'placeholder="e.g. LP1 Projects"'
  )
);

patch("public/index.html", "LP1 role placeholder", content =>
  replace(
    content,
    "inp('pe-role',pe.role,'e.g. Sr. Engineer, Product Team')",
    "inp('pe-role',pe.role,'e.g. Sr. Engineer, LP1 TSMS')"
  )
);

patch("public/index.html", "pharma/Lilly AI risk prompt", content =>
  replace(
    content,
    "You are a project risk analyst reviewing a project portfolio.\\n\\n",
    "You are a project risk analyst reviewing a project portfolio for a pharma/manufacturing science team at Lilly.\\n\\n"
  )
);
...
patch("package.json", "personal name/description/author/appId/branding", content => {
  ...
  pkg.description = "Richard's Projects — LP1 TSMS project management, goals tracking, and progress reporting";
  pkg.author = "Richard — LP1 TSMS";
